<div>
    <x-card>
        <div class="flex items-center justify-between">
            <h1 class="mb-2 font-medium text-2xl">Cadastro de Tags</h1>
            ...
            @interact('column_action', $row)
                <div class="flex gap-1">
                    <x-button.circle icon="pencil" color="sky"
                        wire:click="$dispatch('load::tag', { 'tag' : '{{ $row->id }}'})" outline />
                    <livewire:tags.delete :tag="$row" :key="uniqid('', true)" @deleted="$refresh" />
                </div>
            @endinteract
        </x-table>
        <livewire:tags.update @updated="$refresh" />
    </x-card>
</div>
